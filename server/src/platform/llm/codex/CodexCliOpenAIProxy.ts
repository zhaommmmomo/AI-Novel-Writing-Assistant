...
export interface CodexCliProxyConnection {
  baseURL: string;
  apiKey: string;
}
...
function parseChatRequest(body: ChatCompletionBody): ParsedChatRequest {
  if (typeof body.model !== "string" || !MODEL_PATTERN.test(body.model)) {
    throw new HttpError("model 格式不正确。", 400, "invalid_model");
  }
  if (body.n !== undefined && body.n !== 1) {
    throw new HttpError("Codex CLI 适配器仅支持 n=1。", 400, "unsupported_parameter");
  }
  if (Array.isArray(body.tools) && body.tools.length > 0) {
    throw new HttpError("Codex CLI 适配器不支持模型工具调用。", 400, "unsupported_tools");
  }
...
function usagePayload(usage: CodexTokenUsageBreakdown | null) {
  if (!usage) {
    return undefined;
  }
  return {
...
function sendError(res: ServerResponse, error: unknown): void {
  ...
  const code = error instanceof HttpError ? error.code : "codex_cli_error";
...
export class CodexCliOpenAIProxy {
  private server: Server | null = null;
  private readonly apiKey = randomBytes(32).toString("base64url");

  constructor(private readonly client: CodexAppServerLike = new CodexAppServerClient()) {}

  async start(): Promise<CodexCliProxyConnection> {
    if (this.server) {
    ...
    const server = createServer((req, res) => {
      void this.handleRequest(req, res).catch((error) => {
        ...
        res.destroy(error instanceof Error ? error : new Error("Codex CLI 调用失败。"));
      });
    });
    this.server = server;
  ...
  private connection(): CodexCliProxyConnection {
    if (!this.server) {
      throw new Error("Codex CLI 本地适配器尚未启动。");
    }
    const address = this.server.address() as AddressInfo | null;
    if (!address || typeof address.port !== "number") {
      throw new Error("Codex CLI 本地适配器未获得监听端口。");
    }
    return {
  ...
  private async handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (req.method === "GET" && url.pathname === "/v1/models") {
      ...
      sendJson(res, 200, {
        object: "list",
        data: models.map((id) => ({ id, object: "model", owned_by: "codex-cli" })),
      });
      return;
    }
  ...
  ): { request: CodexGenerationRequest; cleanup: () => void } {
