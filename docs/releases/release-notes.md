...
### 2026-08-19（Claude Code CLI 模型接入）
...
- Claude Code 默认单并发运行，降低长链路同时占用多个订阅任务导致限流或中断的概率。
- Claude Code 只用于文本生成，不会出现在图片生成和知识库向量模型选项中。
- 生成过程中取消任务时会一并停止本机 Claude Code 调用，不会留下继续消耗额度的后台进程。
- Claude Code 调用只在临时目录内运行，不加载本机的项目配置、技能和工具，也不会读写你的代码仓库。
- 修复后台服务方式启动时找不到 Claude Code 命令的问题：之前在终端里能正常运行，但通过服务脚本启动后，设置页刷新 Claude Code 模型列表会直接失败。

### 2026-08-18（模型调用稳定性）

- 文本模型请求的默认等待上限提高到 20 分钟，世界生成、结构化规划等耗时任务不再因为固定 2 分钟限制过早失败；仍可按具体任务显式设置独立上限。
- Codex 长任务会通过 CLI 运行状态检查判断模型是否仍在处理：短暂无输出时继续等待，持续停滞、控制面异常或达到绝对上限时才中断，减少正常慢响应被误判为故障。
...
- Codex 可以参与自动导演、正文生成、审阅、修复和结构化规划，现有小说生产流程无需切换到独立工具。
- 模型列表从本机 Codex CLI 动态读取，默认使用 `gpt-5.6-sol` 与 `max` 推理强度；普通文本和 JSON 结构化输出都可通过设置页连接测试。
- Codex 默认单并发运行，降低长链路同时占用多个订阅任务导致限流或中断的概率。
- Codex 只用于文本生成，不会出现在图片生成和知识库向量模型选项中。
- 长篇章节、章节修复、章节编辑器 AI 改写候选、短篇片段和短篇 AI 修订都会强制经过 Humanizer-zh；处理只调整表达层，并保护事件事实、人物关系、叙事视角和因果顺序。
- Humanizer 失败、返回说明文字或导致正文篇幅明显漂移时，系统不会把未经处理的版本标为成稿。书级开关只控制额外 AI 味检测，不能关闭基础 Humanizer。
